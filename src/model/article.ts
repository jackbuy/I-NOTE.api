...
export const articleDetail  = (req: any, res: any) => {
    ...
    .then((resp: any) => {
        ...
        return Article.updateOne(query, { viewCount: viewCount + 1 })
    })
    .then(() => {
        return Article.findOne(query).populate({path: 'userId', model: User, select: 'username'})
    })
    .then((resp: any) => {
...
export const articleSupport = (req: any, res: any) => {
    ...
    Support
    .findOne( { articleId: articleId, createUserId: userId } )
    .then((resp1) => {
        if (!resp1) {
